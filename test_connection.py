...
def main():
    ...
    # ============================================
    print("\n[*] Paso 3: Verificando credenciales de PostgreSQL...")

    if config.DB_HOST:
        print_success("DB_HOST configurada")
        print_info(f"Host: {config.DB_HOST}")
    else:
        print_error("DB_HOST no configurada")
        return False

    if config.DB_NAME:
        print_success("DB_NAME configurada")
        print_info(f"Database: {config.DB_NAME}")
    else:
        print_error("DB_NAME no configurada")
        return False

    if config.DB_USER:
        print_success("DB_USER configurada")
        print_info(f"User: {config.DB_USER}")
    else:
        print_error("DB_USER no configurada")
        return False
    ...
    try:
        ...
        if users:
            ...
            print_info("Verifica que ejecutaste el script docs/database_schema.sql", indent=4)
    ...
    try:
        ...
        if admin_users:
            ...
            print_info(f"Activo: {'Si' if admin['is_active'] else 'No'}")
        else:
    ...
    tables_to_check = [
        'users', 'members', 'membership_plans', 'member_memberships',
        'payments', 'attendance', 'instructors', 'classes'
    ]
    ...
    if tables_ok < len(tables_to_check):
        ...
        print_info("Asegúrate de ejecutar el script docs/database_schema.sql completo", indent=4)
    ...
    print_success("Conexion a PostgreSQL: OK")
