...
class DashboardSummary:
    """Aggregated KPI data displayed on the Dashboard module."""
    active_members: int = 0
    monthly_revenue: float = 0.0
    today_checkins: int = 0
    expiring_memberships_count: int = 0
    today_schedules: List[ClassSchedule] = field(default_factory=list)
    expiring_memberships: List[MemberMembership] = field(default_factory=list)


@dataclass
class FinancialReport:
    """Aggregated revenue data for the Reports module — Financial tab."""
    total_revenue: float = 0.0
    transaction_count: int = 0
    revenue_by_method: dict = field(default_factory=dict)
...
class MembershipReport:
    """Aggregated membership data for the Reports module — Memberships tab."""
    counts_by_status: dict = field(default_factory=dict)
    counts_by_plan: dict = field(default_factory=dict)
    # Approximation: active / (active + expired). Not a true renewal rate —
    # the schema has no flag distinguishing a renewal from a fresh signup.
    retention_rate_pct: Optional[float] = None
    expiring_soon_count: int = 0
    expiring_soon: List[MemberMembership] = field(default_factory=list)


@dataclass
class AttendanceReport:
    """Aggregated check-in data for the Reports module — Attendance tab."""
    total_checkins: int = 0
    checkins_by_hour: dict = field(default_factory=dict)
    top_members: List[tuple] = field(default_factory=list)
    daily_counts: dict = field(default_factory=dict)


@dataclass
class OperationalReport:
    """Aggregated equipment/class/instructor data for the Reports module — Operational tab."""
    equipment_by_condition: dict = field(default_factory=dict)
    equipment_due_maintenance: List[Equipment] = field(default_factory=list)
    class_occupancy: List[tuple] = field(default_factory=list)
    instructor_workload: dict = field(default_factory=dict)
